<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
          <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">
              إدارة محادثاتك ورسائلك مع فريق العمل والمستفيدين
            </p>
          </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
              <div class="p-2 bg-blue-100 dark:bg-blue-900 rounded-lg">
                <span class="material-icons text-blue-600 dark:text-blue-400">chat</span>
              </div>
              <div>
                <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.totalConversations }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">المحادثات</p>
              </div>
            </div>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
              <div class="p-2 bg-green-100 dark:bg-green-900 rounded-lg">
                <span class="material-icons text-green-600 dark:text-green-400">mark_email_read</span>
              </div>
              <div>
                <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.unreadMessages }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">رسائل غير مقروءة</p>
              </div>
            </div>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.activeChats }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">محادثات نشطة</p>
              </div>
            </div>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.avgResponseTime }}m</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">متوسط وقت الرد</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="lg:col-span-1 border-l border-gray-200 dark:border-gray-700 flex flex-col">
              ...
              <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="flex gap-1">
                  <button
                    @click="setFilter('all')"
                    class="flex-1 px-3 py-1 text-xs rounded-lg transition-colors"
                    :class="currentFilter === 'all' ? 'bg-[#1B3C53] text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'"
                  >
                    الكل
                  </button>
                  <button
                    ...
                    class="flex-1 px-3 py-1 text-xs rounded-lg transition-colors"
                    :class="currentFilter === 'unread' ? 'bg-[#1B3C53] text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'"
                  >
                    غير مقروء
                  </button>
                  <button
                    ...
                    class="flex-1 px-3 py-1 text-xs rounded-lg transition-colors"
                    :class="currentFilter === 'important' ? 'bg-[#1B3C53] text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'"
                  >
                    مهم
                  </button>
                </div>
              ...
              <div class="flex-1 overflow-y-auto">
                ...
                >
                  ...
                  <button
                    @click="showNewMessageModal = true"
                    class="mt-2 text-[#1B3C53] dark:text-blue-400 hover:underline text-sm"
                  >
                    ابدأ محادثة جديدة
                  </button>
                </div>
                      ...
                      <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-1">
                          <span 
                            ...
                            class="text-yellow-500 material-icons text-sm"
                            title="مهم"
                          >star</span>
                          <span
            ...
            <div class="lg:col-span-3 flex flex-col">
                ...
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center">
                  ...
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      :class="selectedConversation.important ? 'text-yellow-500' : 'hover:text-yellow-500'"
                      :title="selectedConversation.important ? 'إزالة التمييز' : 'تمييز كمهم'"
                    >
                      <span class="material-icons">
                        {{ selectedConversation.important ? 'star' : 'star_border' }}
                      </span>
                    </button>
                    <button
                    ...
                    <button
                      @click="showConversationInfo = true"
              ...
              >
                ...
                <p class="text-gray-500 dark:text-gray-500 max-w-md">
                  اختر محادثة من القائمة لبدء المراسلة أو ابدأ محادثة جديدة مع فريق العمل أو المستفيدين
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="p-6">
          <div class="mb-4">
            ...
            >
              ...
              <optgroup label="فريق العمل">
                <option
                  v-for="user in adminRecipients"
                  :key="user.id"
                  :value="user.id"
                >
              ...
              <optgroup label="المستفيدين">
                <option
                  v-for="user in userRecipients"
                  :key="user.id"
                  :value="user.id"
                >
                  {{ user.name }} ({{ getUserRole(user.role) }})
    ...
    <!-- نافذة معلومات المحادثة -->
    <div
      v-if="showConversationInfo && selectedConversation"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
    >
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md">
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            @click="showConversationInfo = false"
            class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
          >
            <span class="material-icons">close</span>
          </button>
        ...
        <div class="p-6 space-y-4">
          <div class="text-center">
            <div class="w-20 h-20 rounded-full flex items-center justify-center text-white text-xl font-semibold mx-auto mb-3"
                 :class="getUserColor(selectedConversation.participant.role)">
              {{ getInitials(selectedConversation.participant.name) }}
            </div>
            <h4 class="font-semibold text-gray-800 dark:text-white">{{ selectedConversation.participant.name }}</h4>
            <p class="text-sm text-gray-500">{{ getUserRole(selectedConversation.participant.role) }}</p>
          </div>

          <div class="grid grid-cols-2 gap-4 text-sm">
            <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p class="text-gray-600 dark:text-gray-400">عدد الرسائل</p>
              <p class="font-semibold text-gray-800 dark:text-white">{{ selectedConversation.messages.length }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
              <p class="text-gray-600 dark:text-gray-400">تاريخ البدء</p>
              <p class="font-semibold text-gray-800 dark:text-white">{{ formatMessageDate(selectedConversation.messages[0]?.timestamp) }}</p>
            </div>
          </div>
          ...
          <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
            <button
              ...
              class="w-full text-center text-[#1B3C53] dark:text-blue-400 hover:underline text-sm py-2"
            >
              تصدير المحادثة
            </button>
          </div>
...
export default {
  ...
  setup() {
    ...
    const stats = computed(() => {
      ...
      const unreadMessages = conversations.value.reduce((sum, conv) => sum + conv.unreadCount, 0)
      const activeChats = conversations.value.filter(conv => 
      ...
      return {
        ...
        unreadMessages,
        activeChats,
        avgResponseTime: 12 // محاكاة - في التطبيق الحقيقي يتم حسابها من البيانات
      }
    })
    ...
    const filteredConversations = computed(() => {
      ...
      if (searchQuery.value) {
        ...
        filtered = filtered.filter(conversation => 
          ...
          getUserRole(conversation.participant.role).toLowerCase().includes(query)
        )
      }
      ...
      if (currentFilter.value === 'unread') {
        ...
        filtered = filtered.filter(conv => conv.important)
      }
    ...
    const adminRecipients = computed(() => 
      availableRecipients.value.filter(user => user.role === 'admin' || user.role === 'employee')
    )

    const userRecipients = computed(() => 
      availableRecipients.value.filter(user => user.role === 'user')
    )

    // تحميل المحادثات
    const loadConversations = () => {
      const storageKey = `conversations_${currentUser.id}`
    ...
    const createNewConversation = () => {
      ...
      const recipient = availableRecipients.value.find(u => u.id === newMessageRecipient.value)
      if (!recipient) return
      ...
      const conversation = {
        ...
        important: markAsImportant.value
      }
    ...
    const toggleImportant = () => {
      if (selectedConversation.value) {
        selectedConversation.value.important = !selectedConversation.value.important
        saveConversations()
      }
    }
    ...
    const exportConversation = () => {
      ...
      const conversationData = {
        participant: selectedConversation.value.participant.name,
      ...
      a.href = url
      a.download = `conversation-${selectedConversation.value.id}.json`
      a.click()
