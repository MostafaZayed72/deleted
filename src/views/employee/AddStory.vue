...
export default {
  ...
  setup() {
    ...
    const isEditing = computed(() => {
      return route.name === 'EditStory' && route.params.id
    })
    ...
    // تحميل بيانات المحتوى إذا كان في وضع التعديل
    const loadStoryData = (storyId) => {
      try {
        // استخدام مفتاح التخزين الموحد 'libraryStories'
    ...
    // معاينة الملف
    const previewFile = () => {
      if (form.value.story_file && form.value.story_file.data) {
        previewUrl.value = form.value.story_file.data
    ...
    const submitForm = async () => {
      ...
      if (!form.value.story_file && !isEditing.value) {
      ...
      try {
        ...
        if (isEditing.value) {
          ...
          const newStory = {
            ...
            submitted_at: form.value.status === 'pending_review' ? now : null,
            reviewed_by: null,
            review_notes: null
          }
          ...
          // إرسال إشعار للمدير
          if (form.value.status === 'pending_review') {
            sendAdminNotification('content_submitted', newStory)
          }

          const message = form.value.status === 'published' ? 'تم نشر المحتوى بنجاح!' : 'تم حفظ المحتوى كمسودة بنجاح!'
          alert(message)
        }

  // توجيه العائدة للموظف
  router.push('/employee/stories')
  try { window.dispatchEvent(new Event('libraryStoriesUpdated')) } catch (e) { /* ignore */ }
...
@font-face {
  ...
  font-weight: 400;
  font-display: swap;
  src: url('https://fonts.googleapis.com/css2?family=Tajawal:wght@300;400;500;700;800&display=swap');
}
</style>